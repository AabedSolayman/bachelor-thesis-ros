# -*- coding: utf-8 -*-
# Quellcode aus github.com/undera/pylgbst übernommen.
# 
# S. Mack, 21.3.2020
...
import math
import traceback
import queue # Hier wird Queue als FIFO-Speicher verwendet, fÃ¼r jede Peripherie eine Queue
from time import sleep
from struct import pack, unpack
from threading import Thread
from pylegoboost import constants as C
from pylegoboost.utilities import str2hex,usbyte,ushort

# Elternklasse fÃ¼r die Peripherie allgemein. Vererbung dann auf die konkrete Peripherie wie Motoren, Sensoren
class Peripheral(object):
    ...
    def __init__(self, parent, port): # Parameter parent vermutlich nÃ¶tig, wenn mehrere MoveHubs
        """
        :type parent: pylgbst.movehub.MoveHub
        :type port: int
        """
        super(Peripheral, self).__init__() # Wozu nÃ¶eig? Wird das nicht automatisch aufgerufen?
        self.parent = parent
        self.port = port
        self._working = False # Attribut Aktionsstatus Periph., setzen mitr Methoden started(), finished()
        self._subscribers = set() # Set mit Callbackfunktionen aller Abos Peripherie-Notifications
        self._port_subscription_mode = None
        # TODO: maybe max queue len of 2?
        self._incoming_port_data = queue.Queue(1)  # FIFO Queue mit nur einem Speichplatz
    ...
    def __repr__(self):
        return "%s on port %s" % (self.__class__.__name__, C.PORTS[self.port] if self.port in C.PORTS else self.port)

    # Befehl an MoveHub senden fÃ¼r Peripherie als Aktor (LED und Motoren)
    def _write_to_hub(self, msg_type, params):
        cmd = pack("<B", self.port) + params
        self.parent.send(msg_type, cmd)
        
    # Parametriert den Sensor, wenn dessen Daten abonniert werden
    def _port_subscribe(self, mode, granularity, enable):
        ...
        self._write_to_hub(C.MSG_SENSOR_SUBSCRIBE, params)
    
    # Peripheriestatus auf beschÃ¤ftigt setzen
    def started(self):
        logging.debug("Peripheral Started: %s", self)
        self._working = True
    
    # Peripheriestatus auf Leerlauf setzen
    def finished(self):
        logging.debug("Peripheral Finished: %s", self)
        self._working = False
        
    # Abfrage Peripheriestatus
    def in_progress(self):
        return bool(self._working)
    
    # Callback fÃ¼r diese Periferie einrichten
    ...
    # Callback fÃ¼r diese Peripherie entfernen
    def unsubscribe(self, callback=None):
        if callback in self._subscribers:
            self._subscribers.remove(callback)

        if self._port_subscription_mode is None:
            logging.warning("Attempt to unsubscribe while never subscribed: %s", self)
        elif not self._subscribers:
            self._port_subscribe(self._port_subscription_mode, 0, False)
            self._port_subscription_mode = None

    # Peripherie gibt verarbeitete Daten an (u.U. mehrere) Subsciber (=Callbackfunktionen) aus
    def _notify_subscribers(self, *args, **kwargs):
        for subscriber in self._subscribers:
            subscriber(*args, **kwargs)

    # Vom MoveHub empfangene Daten in die Queue einlesen
    ...
    # Von der Queue ausgelesene Daten interpretieren und an Subscriber senden
    def handle_port_data(self, data):
        logging.warning("Unhandled device notification for %s: %s", self, str2hex(data[4:]))
        self._notify_subscribers(data[4:])

    # fÃ¼r jede Peripherie separater Thread, liest Queue aus und interpretiert deren Daten.
    def _queue_reader(self):
        while True:
            ...
            try:
                ...
                logging.warning("Failed to handle port data by %s: %s", self, str2hex(data))
    
    # Warteschleife, solange Peripherie beschÃ¤ftigt
    def _wait_sync(self, async):
        if not async:
            logging.debug("Waiting for sync command work to finish...")
            while self.in_progress():
                sleep(0.001)
            logging.debug("Command has finished.")
...
class LED(Peripheral):
    ...
    def __init__(self, parent, port):
        ...
        self._last_color_set = C.COLOR_NONE

    def set_color(self, color, do_notify=True):
        if color == C.COLOR_NONE:
            color = C.COLOR_BLACK
        ...
        self.started()
        self._write_to_hub(C.MSG_SET_PORT_VAL, cmd)

    def finished(self):
        super(LED, self).finished()
        logging.debug("LED has changed color to %s", C.COLORS[self._last_color_set])
...
class EncodedMotor(Peripheral):
    ...
    def _speed_abs(self, relative):
        if relative < -1:
            logging.warning("Speed cannot be less than -1")
            relative = -1

        if relative > 1:
            logging.warning("Speed cannot be more than 1")
            relative = 1
    ...
    def _wrap_and_write(self, mtype, params, speed_primary, speed_secondary):
        ...
        self._write_to_hub(C.MSG_SET_PORT_VAL, params)
    ...
    def handle_port_data(self, data):
        if self._port_subscription_mode == self.SENSOR_ANGLE:
            ...
            logging.debug("Got motor sensor data while in unexpected mode: %s", self._port_subscription_mode)
...
class TiltSensor(Peripheral):
    ...
    # nachfolgende Konstanten vielleicht besser in constants.py?
    TRI_BACK = 0x00
    TRI_UP = 0x01
    TRI_DOWN = 0x02
    TRI_LEFT = 0x03
    ...
    def handle_port_data(self, data):
        if self._port_subscription_mode == self.MODE_3AXIS_SIMPLE:
            ...
            #print('Tilt Sensor Data: ', data)
            roll = self._byte2deg(usbyte(data, 4))
            pitch = self._byte2deg(usbyte(data, 5))
            yaw = self._byte2deg(usbyte(data, 6))  # did I get the order right?
            self._notify_subscribers(roll, pitch, yaw)
        else:
            logging.debug("Got tilt sensor data while in unexpected mode: %s", self._port_subscription_mode)
...
class ColorDistanceSensor(Peripheral):
    ...
    def handle_port_data(self, data):
        if self._port_subscription_mode == self.COLOR_DISTANCE_FLOAT:
            ...
            logging.debug("Unhandled data in mode %s: %s", self._port_subscription_mode, str2hex(data))
...
class Voltage(Peripheral):
    ...
    def subscribe(self, callback, mode=MODE1, granularity=1, async=False):
        logging.info('Subscription Voltage Sensor')
...
class Button(Peripheral):
    ...
    def subscribe(self, callback, mode=None, granularity=1, async=False):
        self.started()
        self.parent.send(C.MSG_DEVICE_INFO, pack('<B', C.INFO_BUTTON_STATE) + pack('<B', C.INFO_ACTION_SUBSCRIBE))
        self._wait_sync(async)
    ...
    def unsubscribe(self, callback=None):
        ...
        if not self._subscribers:
            self.parent.send(C.MSG_DEVICE_INFO, pack('<B', C.INFO_BUTTON_STATE) + pack('<B', C.INFO_ACTION_UNSUBSCRIBE))
